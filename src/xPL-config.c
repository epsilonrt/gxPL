...
static void sendServiceConfigList (xPL_Service * theService) {
  ...
  char * configType;
  char * configName;
  xPL_Message * theMessage = xPL_createBroadcastMessage (theService, xPL_MESSAGE_STATUS);
  xPL_ServiceConfigurable * configurable;
  ...
  for (configIndex = 0; configIndex < theService->configCount; configIndex++) {
    ...
    if (configurable->maxValueCount > 1) {
      ...
      allocSize += 3;
      if (configurable->maxValueCount > 9) {
        allocSize++;
      }
      if (configurable->maxValueCount > 99) {
        allocSize++;
      }
      if (configurable->maxValueCount > 999) {
        allocSize++;
      }
      if (configurable->maxValueCount > 9999) {
        allocSize++;
      }
      ...
      if (configName != configurable->itemName) {
        STR_FREE (configName);
      }
    }
...
static bool parseFilter (char * theText, xPL_ServiceFilter * theFilter) {
  ...
  if ( (vendor = strstr (msgType, ".")) == NULL) {
    return FALSE;
  }
  vendor++;

  if ( (device = strstr (vendor, ".")) == NULL) {
    return FALSE;
  }
  device++;

  if ( (instance = strstr (device, ".")) == NULL) {
    return FALSE;
  }
  instance++;

  if ( (schemaClass = strstr (instance, ".")) == NULL) {
    return FALSE;
  }
  schemaClass++;

  if ( (schemaType = strstr (schemaClass, ".")) == NULL) {
    return FALSE;
  }
  schemaType++;
...
static void sendServiceCurrentConfig (xPL_Service * theService) {
  ...
  for (configIndex = 0; configIndex < theService->configCount; configIndex++) {
    ...
    for (valueIndex = 0; valueIndex < configurable->valueCount; valueIndex++) {
      /* Skip NULL values */
      if (configurable->valueList[valueIndex] == NULL) {
        continue;
      }
    ...
    if (valueCount == 0) {
      xPL_addMessageNamedValue (theMessage, configurable->itemName, NULL);
    }
  }
...
static bool parseConfigDefinition (xPL_Service * theService, char * parseValue, int configType) {
  ...
  if (xPL_strncmpIgnoreCase (parseValue, "newconf", nameLength) == 0) {
    return TRUE;
  }
  if (xPL_strncmpIgnoreCase (parseValue, "interval", nameLength) == 0) {
    return TRUE;
  }
  if (xPL_strncmpIgnoreCase (parseValue, "filter", nameLength) == 0) {
    return TRUE;
  }
  if (xPL_strncmpIgnoreCase (parseValue, "group", nameLength) == 0) {
    return TRUE;
  }
...
static void installNewConfig (xPL_Service * theService, xPL_NameValueList * namedValues, bool allowDefinitions) {
  ...
  char * newInstanceID = NULL;
  int newInterval = -1;

  xPL_NameValuePair * theElement;
  ...
  for (elementIndex = 0; elementIndex < elementCount; elementIndex++) {
    if ( (theElement = xPL_getNamedValuePairAt (namedValues, elementIndex)) == NULL) {
      continue;
    }
    ...
    if (xPL_strcmpIgnoreCase (theElement->itemName, "newconf") == 0) {
      /* See if any change */
      if (strcmp (theElement->itemValue, theService->serviceInstanceID) == 0) {
        continue;
      }
    ...
    if (xPL_strcmpIgnoreCase (theElement->itemName, "interval") == 0) {
      /* See if any change */
      if (!xPL_strToInt (theElement->itemValue, &newInterval)) {
        continue;
      }
    ...
    if (xPL_strcmpIgnoreCase (theElement->itemName, "filter") == 0) {
      ...
      if (strlen (theElement->itemValue) == 0) {
        continue;
      }
      ...
      if (!parseFilter (theElement->itemValue, & (theService->messageFilterList[theService->filterCount]))) {
        continue;
      }
    ...
    if (xPL_strcmpIgnoreCase (theElement->itemName, "group") == 0) {
      ...
      if (theService->groupCount == theService->groupAllocCount) {
        ...
        theService->groupList = (char * *) realloc (theService->groupList, sizeof (char *) * theService->groupAllocCount);
      }
  ...
  /* Preserve settings */
  if (!allowDefinitions) {
    xPL_saveServiceConfig (theService);
  }
  ...
  if (serviceWasEnabled && !allowDefinitions) {
    xPL_dispatchServiceConfigChangedEvent (theService);
  }
}
...
static void xPL_setServiceConfigFile (xPL_Service * theService, char * localConfigFile) {
  ...
  if (theService->serviceEnabled
      || ( (theService->configFileName != NULL) && (localConfigFile != NULL)
           && (strcmp (theService->configFileName, localConfigFile) == 0))) {
    return;
  }
  ...
  if (localConfigFile != NULL) {
    theService->configFileName = xPL_StrDup (localConfigFile);
  }
}
...
static bool xPL_loadServiceConfig (xPL_Service * theService) {
  ...
  *readDevice++ = '\0';
  ...
  for (;;) {
    ...
    /* Find the delimiter */
    if ( (delimstr = strstr (lineBuffer, "=")) == NULL) {
      continue;
    }
    ...
    *delimstr++ = '\0';
    ...
    /* Add the line */
    xPL_addNamedValue (namedValues, lineBuffer, delimstr);
  }
...
static bool xPL_saveServiceConfig (xPL_Service * theService) {
  ...
  xPL_ServiceConfigurable * configItem;
  FILE *configFile;

  /* Skip unless we have a local config file */
  if (theService->configFileName == NULL) {
    return FALSE;
  }
...
xPL_Service * xPL_createConfigurableService (char * vendorName, char * deviceID, char * localConfigFile) {
  ...
  if (localConfigFile != NULL) {
    xPL_setServiceConfigFile (theService, localConfigFile);
  }
  ...
  if (theService->configFileName != NULL) {
    xPL_loadServiceConfig (theService);
  }
...
bool xPL_addServiceConfigValue (xPL_Service * theService, char * itemName, char * itemValue) {
  ...
  if (theItem->valueCount >= theItem->maxValueCount) {
    return FALSE;
  }
  ...
  if (theItem->valueCount == theItem->valueAllocCount) {
    if (theItem->maxValueCount == 1) {
      ...
      if (growValueListBy > 8) {
        growValueListBy = 8;
      }
    }

    theItem->valueAllocCount += growValueListBy;
    theItem->valueList = realloc (theItem->valueList, sizeof (char *) * theItem->valueAllocCount);
  }
