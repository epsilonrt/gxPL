...
static xPL_Message * createHeartbeatMessage (xPL_Service * theService, HeartbeatType heartbeatType) {
...
static xPL_Service * newService (char * theVendor, char * theDeviceID, char * theInstanceID) {
...
static bool doesFilterMatch (xPL_ServiceFilter * theFilter, xPL_Message * theMessage) {
  ...
  if ( (theFilter->matchOnMessageType != xPL_MESSAGE_ANY) && (theFilter->matchOnMessageType != theMessage->messageType)) {
    return FALSE;
  }

  /* If there is a vendor filter and it doesn't match the message, skip the message */
  if ( (theFilter->matchOnVendor != NULL) && (xPL_strcmpIgnoreCase (theFilter->matchOnVendor, theMessage->sourceVendor) != 0)) {
    return FALSE;
  }

  /* If there is a device ID filter and it doesn't match the message, skip the message */
  if ( (theFilter->matchOnDeviceID != NULL) && (xPL_strcmpIgnoreCase (theFilter->matchOnDeviceID, theMessage->sourceDeviceID) != 0)) {
    return FALSE;
  }

  /* If there is a instance ID filter and it doesn't match the message, skip the message */
  if ( (theFilter->matchOnInstanceID != NULL) && (xPL_strcmpIgnoreCase (theFilter->matchOnInstanceID, theMessage->sourceInstanceID) != 0)) {
    return FALSE;
  }

  /* If there is a schema class filter and it doesn't match the message, skip the message */
  if ( (theFilter->matchOnSchemaClass != NULL) && (xPL_strcmpIgnoreCase (theFilter->matchOnSchemaClass, theMessage->schemaClass) != 0)) {
    return FALSE;
  }

  /* If there is a schema type filter and it doesn't match the message, skip the message */
  if ( (theFilter->matchOnSchemaType != NULL) && (xPL_strcmpIgnoreCase (theFilter->matchOnSchemaType, theMessage->schemaType) != 0)) {
    return FALSE;
  }
