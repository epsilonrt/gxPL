/**
 * @file
 * High level interface to manage configurable devices (avr 8-bits source code)
 *
 ...
 */
#include "config.h"
#if CONFIG_DEVICE_CONFIGURABLE && defined(__AVR__)
/* ========================================================================== */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#define DEFAULT_LINE_BUFSIZE 64
#endif
...
/* api functions ============================================================ */
xVector *
gxPLConfigReadFile (UNUSED_ARG (const char *, filename),
  ...
  if ( (file = xEeFileOpen (ucBufferEe, sizeof(ucBufferEe), O_RDONLY)) == NULL) {
...
gxPLDeviceConfigSave (const gxPLDevice * device) {
  ...
  if ( (file = xEeFileOpen (ucBufferEe, sizeof(ucBufferEe), O_WRONLY)) == NULL) {
